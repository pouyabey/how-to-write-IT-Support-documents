# 📘 How-To Guide Template

## 📝 Title

How to [write the task here, like connect to Wi-Fi]

## 🎯 Purpose

The purpose of this guide is to help users [complete the task or solve the issue] by following clear step-by-step instructions.

## Recommended How-To Guide Template

A useful reference for this document is the Atlassian How-to Article Template. This template is designed for writing clear step-by-step instructions that help users complete a task or solve a common issue.

Atlassian How-to Article Template:
https://www.atlassian.com/software/confluence/templates/how-to-article

Although this template is designed for Confluence, its structure can also be used as a reference for GitHub Markdown documentation, internal IT documentation, or help desk knowledge base articles.


## 👥 Audience

This guide is intended for [end users / employees / junior IT technicians].

## ✅ Requirements

Before you begin, make sure you have:

- [Requirement 1, ex: you must have a valid username and password]
- [Requirement 2, ex: you must be a current employee using company owned device]

## 🧭 Steps

### 1. [Step 1, ex: click on the correct SSID which is "pouyabey"]

**Expected result:**  
Describe expected result of step 1.

**Screenshot:**  
Put screenshot here.

---

### 2. [Step 2, ex: enter your company provided username and password]

**Expected result:**  
Describe expected result of step 2.

**Screenshot:**  
Put screenshot here.

---

### 3. [Step 3, ex: read the agreement and sign it]

**Expected result:**  
Describe expected result of step 3.

**Screenshot:**  
Put screenshot here.

---

## 🏁 Final Expected Result

After completing these steps, you should be able to [describe the expected result].

## 🛠️ Troubleshooting

List common issues here.

If you experience issues, try the following:

- [Troubleshooting tip 1]
- [Troubleshooting tip 2]
- [Troubleshooting tip 3]

## 🗒️ Notes

- When to escalate
- How to perform the final verification
